<?php load_header() ?>
<h1>Online Education Program <?php echo anchor('lasrmetrics/online_education/new', 'Add New', array('class' => 'button ui-helper-reset')); ?></h1>

<div class="ui-block wide">
	<h3><a href="#">&nbsp;</a></h3>
	<div>
		<?php echo form_open('lasrmetrics/online_education', array('method' => 'get')) ?>
		...
		<?php echo form_open('lasrmetrics/online_education') ?>
		...
		<table cellpadding="0" cellspacing="0" border="0">
			...
			<tbody>
				<?php if(isset($items) && sizeof($items) > 0): foreach($items as $online_education_program): ?>
				<?php $user = $this->users_model->get_user($online_education_program->created_by); ?>
				<tr class="ui-helper-reset">
					<td><?php echo date_for_display($online_education_program->date); ?></td>
					<td><?php echo $online_education_program->office ?></td>
					<td><?php echo $online_education_program->quarter ?></td>
					<td><?php echo $online_education_program->fiscial_year ?></td>
					<td><?php echo substr($online_education_program->name_of_education_program, 0, 50); ?></td>
					<td><?php echo $user->meta['first_name'].' '.$user->meta['last_name']; ?></td>

					<td>
						<?php echo anchor('lasrmetrics/online_education/edit/'.$online_education_program->ID, 'edit'); ?>/<?php echo anchor('lasrmetrics/online_education/delete/'.$online_education_program->ID, 'delete', array('class' => 'delete')); ?>
					</td>
				</tr>
				<?php endforeach; else: ?>
				<tr class="ui-helper-reset">
