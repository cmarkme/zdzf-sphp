...
<?php endif ?>
<h1>Add Role <?php echo anchor('users/add_role', 'Add New', array('class' => 'button ui-helper-reset')); ?></h1>

<?php echo form_open(site_url('users/add_role/')) ?>
<div class="ui-block wide">
	<h3><a href="#">New role settings</a></h3>
	<div>
		<table cellpadding="0" cellspacing="0" border="0">
			<tbody>
				<tr>
					<th><label for="name">Role Name</label></th>
					<td colspan="3"><?php echo form_input('name'); ?></td>
				</tr>
				...
				<tr>
					...
					<td style="vertical-align: top;">
						<ul class="no-bullets">
							<li>
								<?php echo form_checkbox('caps[]', $cap['controller']); ?>&nbsp;<strong><?php echo $cap['name'] ?></strong>
								<?php if (isset($cap['subs']) && is_array($cap['subs'])): ?>
								<ul>
									<?php foreach ($cap['subs'] as $controller => $name): ?>
									<li><?php echo form_checkbox('caps[]', $controller); ?>&nbsp;<?php echo $name ?></li>
									<?php endforeach ?>
								</ul>
								<?php endif ?>
							</li>
